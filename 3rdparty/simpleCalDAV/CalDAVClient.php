  ...
  function ParseResponse( $response ) {
      ...
      if ($pos === false) {
          ...
          if ( xml_parse_into_struct( $parser, $this->xmlResponse, $this->xmlnodes, $this->xmltags ) === 0 ) {
              ...
              //printf( "\nXML Reponse:\n%s\n", $this->xmlResponse );
              SimpleCalDAVClient_log_message('ERROR', 'XML parsing error: ' 
...
log::add('caldav', 'info', 'CURLOPT_POSTFIELDS '.$this->body);
      ...
      if (FALSE === $response) {
          ...
          SimpleCalDAVClient_log_message('ERROR', 'Error requesting ' . $url . ': ' 
                  . curl_error($this->ch));
          return false;
      }
      ...
      /*
      SimpleCalDAVClient_log_message('INTERNALS', 'REQh: ' . var_export($info['request_header'], TRUE));
      SimpleCalDAVClient_log_message('INTERNALS', 'REQb: ' . var_export($this->body, TRUE));
      SimpleCalDAVClient_log_message('INTERNALS', 'RPLh: ' . var_export($this->httpResponseHeaders, TRUE));
      SimpleCalDAVClient_log_message('INTERNALS', 'RPLb: ' . var_export($this->httpResponseBody, TRUE));
      */
